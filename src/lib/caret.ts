...
export function caret(node: Element): ActionReturn<undefined, Attributes> {
	...
	const updateCursorXY = () => {
		// get current cursor position as character index
		const selectionPoint = node.selectionEnd ?? 0;

		// get x and y coordinates of input element relative to its offsetParent (parent with non-static position)
		...
		const event = new CustomEvent('caretmove', {
			detail: {
				...
				spanRect: new DOMRect(spanX, spanY, spanWidth, spanHeight),
				selection: {
